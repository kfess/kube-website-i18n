...
if [ -z "$INPUT_FILE" ]; then
    INPUT_FILE="$PROJECT_ROOT/data/master/git_history.csv"
fi
...
OUTPUT_DIR="$PROJECT_ROOT/data/derived/classification"
...
tail -n +2 "$INPUT_FILE" | while IFS=, read -r hash author date message filepath; do
